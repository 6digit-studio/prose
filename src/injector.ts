...
/**
 * Generate usage instructions for agents to use claude-prose
 */
...
export function ensureTemplate(rootPath: string): void {
    ...
    if (existsSync(templatePath)) {
        console.log(`ℹ️  CLAUDE.md.template already exists`);
        return;
    }
    ...
    if (baseContent.includes('{{PROSE_SUMMARY}}')) {
        console.log(`ℹ️  CLAUDE.md already contains prose tags, using as basis for template`);
    } else {
        baseContent += `\n\n## 🧬 Project Consciousness\n{{PROSE_SUMMARY}}\n\n### Decisions\n{{PROSE_DECISIONS}}\n\n### Instructions & Usage\n{{PROSE_INSTRUCTIONS}}\n\n### Active Gotchas\n{{PROSE_GOTCHAS}}\n`;
    }

    writeFileSync(templatePath, baseContent);
    console.log(`✅ Created CLAUDE.md.template from existing CLAUDE.md`);
}
...
export function injectMemory(rootPath: string, memory: ProjectMemory): void {
    ...
    try {
        ...
        console.log(`📝 Updated CLAUDE.md from template`);
    } catch (error: any) {
        console.error(`⚠️  Failed to inject memory into CLAUDE.md: ${error.message}`);
    }
}

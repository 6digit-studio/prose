...
import { loadMemoryIndex, loadProjectMemory, getMemoryDir, type ProjectMemory } from './memory.js';
...
function generateChronicleSection(memory: ProjectMemory, projectName: string): string {
  ...
  const sessionEntries = sorted.map((snapshot, index) => {
...
<body>
  ...
  <section>
    <h2>💭 LLM Musings</h2>
    <div class="musings">${escapeHtml(musings)}</div>
  </section>
  ` : ''}

  <footer>
    <a href="./${projectName}.html">${escapeHtml(shortName)}</a> •
    Generated by <a href="https://github.com/anthropics/claude-code">claude-prose</a>
  </footer>
</body>
</html>
`;
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Claude Prose - Project Memory</title>
  <style>
    :root {
      --bg: #0d1117;
      --bg-secondary: #161b22;
